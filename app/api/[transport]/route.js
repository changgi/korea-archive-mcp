...
8. KARDA 온톨로지 산출물(kad/): 발굴 결과를 관계 그래프 입력으로 변환해 zip에 포함 — karda_nodes.csv(id·type[record/agent/unit/place/event]·label·속성)·karda_edges.csv(source·target·relation·relation_ko·basis·confidence)·kad_readme.txt. 신뢰도 필터: 확정(API·원문 판독)·높음만 그래프 수록, 부분 이하는 tentative로 readme에 명기(그래프 오염 방지). 관계마다 근거(basis)를 남겨 연구 재검증 가능하게. 로컬은 make_metadata.py 활용 가능. 품질 게이트 추가: ①보고서 CSS는 템플릿 전체를 그대로 포함(부분 복사 금지 — 영상 절 등 미사용 스타일도 포함해야 절 추가 시 깨지지 않는다) ②핵심 기록 카드는 수집된 실물이 있으면 반드시 이미지 포함(자료 화면 대체 시 그 사실 명기) ③갤러리·절은 placeholder 주석이 아니라 실제 임베드 — 납품 전 잔존 placeholder 검사.
